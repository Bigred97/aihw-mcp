...
- Add MCP Registry ownership marker (`mcp-name: io.ausdata/aihw-mcp`) to the
...
0.4.13 release CI failed lint because the three-pool ranker refactor
left `from rapidfuzz import fuzz, process` behind. Re-shipping with
the import cleaned up. No runtime change vs 0.4.13.
...
before returning. Gateway customers see no change (gateway re-ranks);
direct-MCP callers (Claude Code etc.) can now order their UI without
