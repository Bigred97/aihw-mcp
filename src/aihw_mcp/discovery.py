...
When AIHW publishes a refreshed GRIM workbook or a new MORT release, the
corresponding data.gov.au resource gets a fresh GUID-based URL (the
package slug typically stays stable — e.g. `grim-books` — but the
underlying file URL changes). Without discovery, the hard-coded YAML
`download_url` would silently keep serving last year's snapshot.
...
The discovery layer fixes this by resolving the URL at query time:
  ...
  4. If discovery fails (network down, AIHW renamed something, no match),
...
from .client import AIHWAPIError, AIHWClient
...
class DiscoverySpec:
    ...
    Args:
        package_id: exact CKAN package slug (e.g. "grim-books"). Use this
            for datasets where every release lives as a resource inside
            the same package — the AIHW common case.
        package_id_pattern: regex with one numeric capture group, applied
            ...
            year and the highest-year match wins. Reserved for future
            AIHW datasets that ship one package per release year.
        organization_id: required when using package_id_pattern. The CKAN
            org slug to enumerate (e.g. "aihw").
        resource_name: exact name match against `resource["name"]`. AIHW
            preserves resource names ("GRIM", "MORT_TABLE_1") across
            yearly file refreshes — this is the most reliable shape.
...
async def resolve_latest_url(client: AIHWClient, spec: DiscoverySpec) -> str:
...
async def _resolve_latest_package_id(
    client: AIHWClient, spec: DiscoverySpec
) -> str:
    if not spec.organization_id:
        raise DiscoveryError(
            "package_id_pattern requires organization_id (e.g. 'aihw')"
        )
