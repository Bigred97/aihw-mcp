"""XLSX and CSV parsers for AIHW resources on data.gov.au.

Two source formats:
  - CSV (the vast majority of AIHW data.gov.au resources: GRIM, MORT, ACIM,
    health expenditure, youth justice, public hospitals) — flat, headers
    on row 1.
  - XLSX (used by some AIHW reports — e.g. mental health data tables on
    aihw.gov.au) — usually one "Notes" sheet of preamble + one or more
    data sheets. Headers land on row 1, 2, or 3 depending on the table;
    the curated YAML pins the exact row.
...
is the openpyxl read time for the bigger files.
"""
...
class ParseError(Exception):
    """Raised when an AIHW resource can't be parsed."""
...
def _normalize_header(c):
    ...
    AIHW headers occasionally carry meaning in embedded newlines (units
    suffix on a separate line) so we keep the newline. But we strip any
    padding whitespace around the newline because reports sometimes ship
    inconsistent whitespace variants of the same logical name across
    years. Normalizing here means curated YAMLs only ever spell one
    canonical form.
    """
...
def drop_blank_rows(df: pd.DataFrame, key_columns: list[str]) -> pd.DataFrame:
    ...
    Use this to trim trailing footnote / blank rows that AIHW sometimes
    leaves after the data block. We require ALL key columns to be NaN
    before discarding — a single non-null in any key column means the
    row is real.
    """
