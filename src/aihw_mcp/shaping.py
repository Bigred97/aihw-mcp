...
Wraps two layout-specific transforms:
  - `shape_wide`:        one-row-per-entity tables (GRIM, MORT, ACIM, public
                         hospitals). Each (row, measure) cell becomes one
                         Observation.
  - `shape_transposed`:  metric-rows × period-columns tables. Each (metric,
                         period) cell becomes one Observation, with a
                         `period` field carried through. No transposed
                         datasets ship in v0.1 — the code path is preserved
                         for future AIHW reports that use it.

In both cases, the public API exposes plain-English column aliases (e.g.
`deaths`, `crude_rate`) instead of AIHW's source headers. Filtering happens
on aliases too.
"""
...
def _apply_aliases(df: pd.DataFrame, cd: CuratedDataset) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(
            ...
            "https://github.com/Bigred97/aihw-mcp/issues."
        )
...
def _to_clean_string(series: pd.Series) -> pd.Series:
    ...
    Two transforms:
    1. Digit-only columns (IDs, postcodes) come from pandas as floats; plain
       `.astype('string')` yields '94600082111.0'. We route whole-number
       floats through Int64 first to drop the trailing '.0'.
    2. Text columns (state, sex) often arrive with trailing/leading
...
def _apply_filters(
    ...
    for user_key, user_val in filters.items():
        ...
        if isinstance(user_val, list):
            ...
            # Use string comparison so numeric IDs and string aliases both match.
            mask = out[user_key].astype("string") == str(resolved)
            unresolved_value = str(user_val).strip()
        next_out = out.loc[mask]
        # High-confidence "Did you mean?" when a filter on a free-form
        # dim wipes all rows. Cutoff 0.7 is strict so genuinely-absent
        # values still pass through silently (matches ato-mcp 0.8.13).
        if next_out.empty and not out.empty:
            ...
            if not has_enum:
                ...
                # Year-shaped dims (4-digit numeric) get RANGE-style hints
                # instead of difflib fuzzy matches — fuzzy on years just
                # picks a similar number ('2022' → '2002'), which is
                # misleading. If the user passed a year outside the
                # published range, tell them the range.
                user_int = None
...
def shape_transposed(
    ...
    # Some AIHW tables ship metric labels with stray whitespace. Normalize
    # once so both filter matching and the response display use clean values,
    # and YAML aliases don't have to mirror typos.
...
def _period_in_range(p: str, start: str | None, end: str | None) -> bool:
    ...
    AIHW periods come in many shapes: 'YYYY' (2023), 'YYYY-YY' (1997-98),
    'YYYY-MM-DD' (rare). We try to extract a year + optional month and
    compare lexicographically. Free-form / unparseable periods always
    pass through.
    """
...
def _normalize_period(p: str) -> str | None:
    ...
    Disambiguation rule for the `YYYY-NN` shape:
      ...
      - if NN is 13-99, it's the YY suffix of a financial year ('1997-98')
