...
from aihw_mcp.cache import Cache, TTL
...
async def test_corrupt_db_silent_rebuild(temp_db: Path):
    ...
    temp_db.write_bytes(b"this is not a sqlite database\x00\xff\xfe")
    cache = Cache(temp_db)
...
def test_ttl_constants_defined():
...
async def test_large_payload_roundtrip(temp_db: Path):
    """Payloads up to 25MB (biggest AIHW file = GRIM) must roundtrip cleanly."""
    cache = Cache(temp_db)
